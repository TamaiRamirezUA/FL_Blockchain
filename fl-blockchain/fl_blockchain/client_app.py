...
from fl_blockchain.task import load_data, load_model, PROJECT_PATH, NUM_CLIENTS
...
class FlowerClient(NumPyClient):
    def __init__(
        self, model, epochs, batch_size, verbose, partition_id, num_partitions
    ):
        self.model = model
    ...
    def fit(self, parameters, config):
        ...
        H = self.model.fit(self.X_train, self.y_train,
            ...
            callbacks=[TensorBoard(log_dir=f"{PROJECT_PATH}/NC_{NUM_CLIENTS}/logs/fit/client_{self.partition_id}/round_{actual_round}")]
        )
        ...
        plot_dir = f"{PROJECT_PATH}/NC_{NUM_CLIENTS}/plots/client_{self.partition_id}/round_{actual_round}"
...
def client_fn(context: Context):
    ...
    return FlowerClient(
        net, epochs, batch_size, verbose, partition_id, num_partitions
    ).to_client()
